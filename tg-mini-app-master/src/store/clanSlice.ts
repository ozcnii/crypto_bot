import { createClanRequest, deleteUserClanRequest, getClanByIdRequest, getClanListRequest, getUserClanRequest, joinUserClanRequest, leaveUserClanRequest } from '@/utils/api/clan'
import { Clan, ClanList, ClanState, CreateClan } from '@/utils/types/clan'
import { createAsyncThunk, createSlice } from '@reduxjs/toolkit'
import { closeModal } from './modalsSlice'
import { showNotification } from './notificationSlice'

export const getUserClan = createAsyncThunk('clan/getUserClan', async (_, { rejectWithValue }) => {
	try {
		const clan = await getUserClanRequest();
		if (clan === null) {
			return rejectWithValue({ status: 404, message: 'Clan not found' });
		}
		return clan;
	} catch (error: any) {
		return rejectWithValue({ status: error.response?.status, message: error.message });
	}
});

export const deleteUserClan = createAsyncThunk('clan/deleteUserClan', async (link: string, { rejectWithValue, dispatch }) => {
	try {
		const clan = await deleteUserClanRequest(link);
		if (clan.status === 200) {
			dispatch(showNotification({ message: "You've deleted clan!", type: 'success', logo: 'img/bear.png' }))
			return clan;
		} else {
			dispatch(showNotification({ message: "Something is wrong..", type: 'error', logo: 'img/brokenhouse.png' }))
...
export const joinUserClan = createAsyncThunk('clan/joinUserClan', async (id: number, { rejectWithValue, dispatch }) => {
	try {
		const clan = await joinUserClanRequest(id);
		if (clan.status === 200) {
			dispatch(showNotification({ message: "You've joined clan!", type: 'success', logo: 'img/house.png' }))
			return clan;
		} else {
			dispatch(showNotification({ message: "Something is wrong..", type: 'error', logo: 'img/brokenhouse.png' }))
			return rejectWithValue({ status: clan.status, message: clan.message });
...
export const leaveUserClan = createAsyncThunk('clan/leaveUserClan', async (_, { rejectWithValue, dispatch }) => {
	try {
		const clan = await leaveUserClanRequest();
		if (clan.status === 200) {
			dispatch(showNotification({ message: "You've left clan!", type: 'success', logo: 'img/bear.png' }))
			return clan;
		} else {
			dispatch(showNotification({ message: "Something is wrong..", type: 'error', logo: 'img/brokenhouse.png' }))
			return rejectWithValue({ status: clan.status, message: clan.message });
...
export const getClanList = createAsyncThunk('clan/getClanList', async (_, { rejectWithValue }) => {
	try {
...
export const getClanById = createAsyncThunk('clan/getClanById', async (id: string | undefined, { rejectWithValue }) => {
	try {
...
export const createClan = createAsyncThunk('clan/createClan', async (link: string, { rejectWithValue, dispatch }) => {
	try {
		const clan = await createClanRequest(link);
		if (clan.status !== 200) {
			dispatch(showNotification({ message: clan.data.message, type: 'error', logo: 'img/brokenhouse.png' }))
			return rejectWithValue({ status: clan.status, message: clan.data.message });
		}
		dispatch(showNotification({ message: 'Clan created', type: 'success', logo: 'img/house.png' }))
		dispatch(closeModal())
		return clan.data;
	} catch (error: any) {
		dispatch(showNotification({ message: "Something is wrong..", type: 'error', logo: 'img/brokenhouse.png' }))
