...
export const Header = () => {
  ...
  const { userLeague, clanLeague } = useSelector(
      ...
      <div className={css.info}>
        ...
        <div className={css.statistics}>
          {user.balance ? (
            ...
            <p className={css.balance}>0</p>
          )}
          <div className={css.divider}>
            <div className={css.league}>
              {userLeague && (
                <img
                  src={`img/leagueLogos/${userLeague.name}.png`}
                  alt="лого лиги"
                  className={css.leagueLogo}
                />
              )}
              <span className={css.leagueName}>
                {userLeague ? userLeague.name : 'No'} League
              </span>
            </div>
            <p className={css.pnl}>
              P&L:{' '}
              {user.p_n_l ? (
                <span className={user.p_n_l > 0 ? css.green : css.red}>
                  {user.p_n_l > 0 ? `+${user.p_n_l}` : user.p_n_l}
                </span>
              ) : (
                '0'
              )}
        ...
        {clan.logo_url ? (
          ...
          <div className={css.noClanLogo}>
            <img
              src="img/noClan.png"
              alt="лого клана"
              className={css.noClanLogoImg}
            />
          </div>
        )}
        <div className={css.clanInfo}>
          <p className={css.clanName}>
            {clan.name ? clan.name : 'No clan yet'}
          </p>
          <div className={css.clanStatistics}>
            ...
            <p className={css.clanLeague}>
              <img
                src={
                  clanLeague.name
                    ? `img/clanLogos/${clanLeague.name}.png`
                    : 'img/clanLogos/Gold.png'
                }
                alt="лого лиги"
                className={css.clanLeagueLogo}
              />
              <span className={css.clanLeagueName}>
                {clanLeague.name ? (
                  clanLeague.name + ' League'
                ) : (
                  <div className={css.noClanLeague}></div>
                )}
              </span>
            </p>
          </div>
